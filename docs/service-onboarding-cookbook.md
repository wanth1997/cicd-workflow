...
- 所有 caller workflow 都要 pin 到 tag，例如 `@v0.5.0`。不要用 moving
  `main`。
...
jobs:
  deploy-dry-run:
    uses: wanth1997/cicd-workflow/.github/workflows/deploy-dry-run-artifact.yml@v0.5.0
    with:
      service_name: example-service
      python_version: "3.11"
      node_version: "24"
...
jobs:
  deploy-staging-runner-local:
    uses: wanth1997/cicd-workflow/.github/workflows/deploy-staging-artifact.yml@v0.5.0
    with:
      service_name: example-service
      python_version: "3.11"
...
jobs:
  ...
  production-preflight:
    ...
    uses: wanth1997/cicd-workflow/.github/workflows/production-preflight-artifact.yml@v0.5.0
    with:
      service_name: example-service
      python_version: "3.11"
      release_run_id: ${{ inputs.release_run_id }}
...
2. 新增 PR CI wrapper，開一支測試 PR，確認 backend fast、migration tests、
   migration check、frontend checks 都跑綠。
3. 記錄 GitHub 顯示的實際 check names，再更新 branch protection。不要沿用舊
   workflow 的 required checks。
4. 新增 Release Candidate wrapper，從 default branch 跑一次。確認 release
   artifact 名稱符合 `<service>-release-*`，且 artifact 裡有 manifest。
...
- 每個 service 應以獨立 PR 升級 workflow tag，例如 `v0.5.0` 到下一個 tag。
